...
int* circularArrayRotation(int a_count, int* a, int k, int queries_count, int* queries, int* result_count) {
    ...
    *result_count = queries_count;
    ...
    int effective_k = k % a_count;

    for (int i = 0; i < queries_count; i++) {
        // Calculate the index in the original array that shifted to queries[i]
        // (target_index - shift + array_length) % array_length
        int original_idx = (queries[i] - effective_k + a_count) % a_count;
        
        results[i] = a[original_idx];
    }
