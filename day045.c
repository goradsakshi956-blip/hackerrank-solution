...
void bonAppetit(int bill_count, int* bill, int k, int b) {
    long total_shared_cost = 0;

    // 1. Calculate total cost of items Anna actually ate
    for (int i = 0; i < bill_count; i++) {
        if (i != k) {
            total_shared_cost += bill[i];
        }
    }

    // 2. Anna's fair share is half of that total
    long anna_actual_share = total_shared_cost / 2;

    // 3. Check if Brian's calculation (b) is correct
