...
int* climbingLeaderboard(int ranked_count, int* ranked, int player_count, int* player, int* result_count) {
    *result_count = player_count;
    ...
    // Step 1: Create a unique list of scores (Dense Ranking)
    // Since ranked is already sorted descending, we just skip duplicates
    ...
    // Step 2: Iterate through player scores
    // Since player scores are ascending, we can move a pointer 
    // backwards through the unique leaderboard once.
    int curr_leaderboard_idx = unique_count - 1;
    for (int i = 0; i < player_count; i++) {
        while (curr_leaderboard_idx >= 0 && player[i] >= unique_ranked[curr_leaderboard_idx]) {
            curr_leaderboard_idx--;
        }
        // Rank is (index + 1) + 1 because of 1-based indexing and 
        // the fact that we stopped one index before the actual position
        results[i] = curr_leaderboard_idx + 2;
    }
