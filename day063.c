...
int jumpingOnClouds(int c_count, int* c, int k) {
    ...
    // Use a do-while loop because the game ends 
    // when you land back on 0, but you must jump first.
    do {
        // Move k steps forward in a circular array
        current = (current + k) % c_count;
        
        // Basic jump cost
        energy -= 1;
        
        // Additional cost if it's a thunderhead
        if (c[current] == 1) {
            energy -= 2;
        }
